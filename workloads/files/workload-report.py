...
def parseMetrics(metrics_dictionary, result_type):
    ...
    for row in (metrics_dictionary['data']['result']):
        ...
        if result_type == 'vector':
            ...
            metrics.append(str(counter) + ',' + str(row['value'][0]) + ',' + str(metric))
        elif result_type == 'matrix':
            print("Metrics matrix lenght: {}".format(len(row['values'])))
            for value in row['values']:
                counter = counter + 1
                metric = round(float(value[1]), 1)
                metrics.append(str(counter) + ',' + str(value[0]) + ',' + str(metric))
    return metrics
...
def generateGraphReport(reportFile, x_axis="SN", y_axis="METRIC_VALUE", title="", file_name=""):
    df1 = pd.read_csv(reportFile)
    fig1 = px.line(df1, x=x_axis, y=y_axis, title=title)
...
def writeToCsvFile(cmd_args, metrics):
    ...
    if os.path.exists(file_path) and os.path.isdir(file_path):
        ...
        with open(file_name, 'w') as csv_file:
            fieldNames = ['SN', 'TIMESTAMP', 'METRIC_VALUE']
            csvWriter = csv.DictWriter(csv_file, fieldnames=fieldNames)
            csvWriter.writeheader() 
            print("{},{}".format(fieldNames[0], fieldNames[1]))
            for i in metrics:
                row = i.split(',')
                csvWriter.writerow({fieldNames[0]: row[0], fieldNames[1]: row[1], fieldNames[2]: row[2]})
...
if __name__ == '__main__':
    ...
    if os.path.isfile(metrics_csv_file):
        ...
        file_name = vars(args).get('file_name').split('.', 1)[0] + '-graph.png'
        image_file = os.path.join(file_path, file_name)
        print("Image File: {}".format(image_file))
        generateGraphReport(metrics_csv_file, title=file_name, file_name=image_file)